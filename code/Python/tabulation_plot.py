...
if (mode == 2):
    ...
    with open("../data/allLO-12.5-250-0.01-0.01.txt") as f:
        lines = f.readlines()
        for line in lines:
            for vals in data:
                vals.append(float((line[: line.find('  ')])))
    ...
    plt.xlim(0.01,1)
    plt.ylim(-10,10)
    plt.title('Evolution from $Q_0 = 125$ to $Q = 250$ GeV ($R = 0.5$)',fontsize=14)
...
if (mode == 2.1):
    ...
    filenames = ["../data/all-250-250-0.01-0.01.txt",
                 "../data/all-247.5-250-0.01-0.01.txt",
                 "../data/all-175-250-0.01-0.01.txt",
                 "../data/all-125-250-0.01-0.01.txt",
                 "../data/all-75-250-0.01-0.01.txt",
                 "../data/all-25-250-0.01-0.01.txt",
                 "../data/all-12.5-250-0.01-0.01.txt"]
    labels = []
...
#        label = label + ", dlnlnQ = " + shortname[: shortname.find(".t")]
        shortname = filename[filename.find("-",12) + 1 :]
        Q0 = filename[12 : filename.find("-",12)]
...
if (mode == 2.2):
    plotfile = "../../paper/figures/LO-gluon.pdf"
    filenames = ["../data/test-250-250-0.01-0.01.txt",
                 "../data/test-247.5-250-0.01-0.01.txt",
                 "../data/test-175-250-0.01-0.01.txt",
                 "../data/test-125-250-0.01-0.01.txt",
                 "../data/test-75-250-0.01-0.01.txt",
                 "../data/test-25-250-0.01-0.01.txt",
                 "../data/test-12.5-250-0.01-0.01.txt"]
    labels = []
    for filename in filenames:
#        label = "Q0 = " + filename[12 : filename.find("-",12)]
#        shortname = filename[filename.find("-",12) + 1 :]
...
#    ##   
    ...
    plt.xlim(0.01,1)
    plt.ylim(-10,10)
    plt.title('LO gluon evolution',fontsize=15)
    xvals = []
    gvals = []
    ...
    for i in range(np.shape(gvals)[0]):
        if not (np.array_equal(gvals[i],np.zeros(np.shape(gvals[i])[0]))):
            plt.plot(xvals[i], gvals[i], '-x', label = labels[i])
    plt.legend(bbox_to_anchor=(1,.3), loc=3)
